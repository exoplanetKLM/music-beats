...
- 首次加载约 5MB（曲库 18 首，但**只下载你当前选中的那一首**，128kbps），**建议用 Wi-Fi**：菜单秒出，稍等几秒即可点「开始游戏」
- 微信里打不开就点右上角「…」→「在浏览器打开」
- iPhone 可以把链接「添加到主屏幕」，之后全屏启动体验最好

### 单文件版（离线备份）

- **安卓手机**：微信/QQ 收到的 `音乐跳动-单文件.html`，用文件管理器打开并选择浏览器即可玩（飞行模式也能玩，内置 5 首歌）
- **电脑**：双击就能玩（文件约 20MB，微信里打开会慢一点，建议用上面的网页版）
- **iPhone**：无法直接打开本地 HTML 文件，请用上面的链接玩

> 单文件版**只内置 5 首**（No.9 / Sugar Free / Sexy Love / 纠缠Shattered / flower.）：18 首全塞进去会让文件涨到约 78MB，手机浏览器打不开、微信也传不动。想玩全部 18 首请用上面的网页版。

玩法：电脑点鼠标 / 按 J（或空格）跳跃、按 A 开护盾，手机点屏幕跳跃——踩中节拍得分（金色星形翻倍），躲开障碍，碰到障碍游戏结束。「音乐选择」弹窗里可切换歌曲（网页版 18 首、单文件版 5 首，每首带歌手名），点一首选中、按「确定」生效，每首歌最高分独立保存。
...
| song-data*.js | 18 首歌的 base64 数据（128kbps 重压版，**勿手改**，按需加载；原始 320kbps 备份在本地 .audio-backup，未入库）。《夜、萤火虫和你》《美梦》两首原曲明显偏轻，转码时额外做过增益+限幅（见 CLAUDE.md「歌曲音量」） |
...
- **打开很慢 / 开始按钮点不了**：当前这首歌的数据还在下载（一首约 4~5MB，按需加载——18 首里只下你选的那一首），切 Wi-Fi 或稍等；超时后会先用合成音乐放行
- **换了首歌听着不一样大**：曲库响度做过统一（以现有曲目均值为基准，偏差都在 4dB 以内），个别曲子风格本身偏安静属于正常
